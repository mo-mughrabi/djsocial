...
from apps.twitter.models import Twitter, Order
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        print Order.objects.filter(func='retweet').where(
            HE("kwargs").contains({'tweet': 'محاكمة خليجية...غازلت زوجها http://dlvr.it/4skNQD  #kuwait #الكويت'}))
